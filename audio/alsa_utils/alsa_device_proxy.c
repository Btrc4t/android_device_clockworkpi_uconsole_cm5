...
static bool is_hdmi(alsa_device_proxy * proxy, const alsa_device_profile* profile) {
    char hdmi_device[PROPERTY_VALUE_MAX];
    char actual_device_path[] = "/proc/asound/card[]/id";
    snprintf(strchr(actual_device_path, '['), 5, "%d/id",  profile->card);
    char actual_device[PROPERTY_VALUE_MAX];
    char default_device[] = "hdmi0";
    char* result = NULL;

    ALOGW("%s: Reading from %s", __func__, actual_device_path);
    FILE *file = fopen(actual_device_path, "rb");
    if (!file) {
        result = (char*) 1;
        ALOGW("%s: Could not open  %s", __func__, actual_device_path);
    } else {
        long file_size = ftell(file);
        size_t len = file_size > PROPERTY_VALUE_MAX ? PROPERTY_VALUE_MAX : file_size;
        fread(actual_device, sizeof(char), len, file);
        actual_device[len - 1] = '\0';
        result = strstr(actual_device, default_device);
        fclose(file);
    }

    if (result) {
        property_get("persist.vendor.audio.device", hdmi_device, default_device);
        ...
        if (!strcmp(hdmi_device, "hdmi0") || !strcmp(hdmi_device, "hdmi1")) {
            ...
            sprintf(proxy->hdmi_device_name, "default:CARD=vc4%s", hdmi_device);
            return true;
        }
    }
    return false;
        ...
        const alsa_device_profile * profile)
{
    ...
    proxy->hdmi = is_hdmi(proxy, profile);
